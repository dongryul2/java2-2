...
- 멀티 스레드의 공유 데이터의 동시 접근문제 해결
> 방법 1) 공유 데이터를 접근하는 모든 스레드의 한줄 세우기  
...
3.GUI 응용프로그램은 이벤트 기반 프로그래밍으로 작성됨
- GUI 라이브러리 종류
> C++ MFC, C# GUI, Visual Basic 등등
...
- 예: ActionListener 인터페이스 , MouseListener 인터페이스

사용자의 이벤트 리스너 작성
- 자바와 리스너 인터페이스를 상속받아 구현
...
2.이벤트 리스너 클래스 작성: ActionListenr 인터페이스 구현

3.이벤트 리스너 등록
- 이벤트를 받아 처리하고자 하는 컴포넌트에 이벤트 리스너 등록
- component.addXXXListner(listner)
...
- 제네릭
> 1. 특정 타입만 다루지 않고, 여러 종류의 타임으로 변신할 수 있도록 클래스나 메소드를 일반화 시키는 기법  
> 2. 클래스나 인터페이스 이름에 <E> <K> <V> 등 타입매개변수 포함
제네릭 컬렉션 사례: 벡터 Vector<E>  
> 3. <E>에서 E에 구체적인 타입을 주어 구체적인 타입만 다루는 벡터로 활용  
> 4. 정수맍 다루는 컬렉션 벡터 Vector<Integer>  
> 5. 문자열만 다루는 컬렉션 벡터 Vector<String>
...
- Vector에 삽입 가능한 것
> 1. 객체,null  
> 2. 기본 타입의 값은 Wrapper 객체로 만들어 저장
- Vector에 객체 삽입
> 벡터의 맨 뒤, 중간에 객체 삽입가능
- Vector에서 객체 삭제
> 임의의 위치에 있는 객체 삭제 가능
...
- AWT 패키지
    ...
    > 2. AWT 컴포넌트는 중량 컴포넌트
- Swing 패키지
    > 1. AWT 기숳을 기반으로 작성된 자바 라이브러리
    > 2. 모든 AWT 기능 + 추가된 풍부하고 화려한 고급 컴포넌트
    > 3. AWT 컴포넌트를 모두 스윙으로 재작성
